...
if compute_gcp == True:
    ...
    kwargs = {
        ...
        'window_size'  : (2000,2000),#modificato(era (128,128) per Katy)
        'max_shift'    : 7000,#modificato (era 10 per Katy),
        ...
        'min_reliability' : 40,#era 40 per Katy
        #'tieP_filter_level': 0,
        'ignore_errors' : True,
        'CPUs'         : options.cores,
        'q'            : quiet,
    ...
    if options.output_tpg != None:
        ...
        #GDF = GDF.drop(columns=['REF_BADDATA','TGT_BADDATA','SSIM_IMPROVED'])
