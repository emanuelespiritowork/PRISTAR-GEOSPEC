...
path = 'D:\\Documenti\\CNR-IBE\\Dati\\PRISMA_new_geocoding_issue\\'
...
b['RELIABILITY'] = np.array(f[['RELIABILIT_x','RELIABILIT_y']])
b['POINT_ID'] = np.array(f['POINT_ID'])
b['X_IM'] = np.array(f[['X_IM_x','X_IM_y']])
b['X_MAP'] = np.array(f[['X_MAP_x','X_MAP_y']])
b['Y_IM'] = np.array(f[['Y_IM_x','Y_IM_y']])
b['Y_MAP'] = np.array(f[['Y_MAP_x','Y_MAP_y']])
b['S2_Band'] = np.array(f[['S2_Band_x','S2_Band_y']])

b['ABS_SHIFT'] = np.array(f[['ABS_SHIFT_x', 'ABS_SHIFT_y']])
b['ANGLE'] = np.array(f[['ANGLE_x', 'ANGLE_y']])
b['L1_OUTLIER'] = np.array(f[['L1_OUTLIER_x','L1_OUTLIER_y' ]])
b['L2_OUTLIER'] = np.array(f[['L2_OUTLIER_x','L2_OUTLIER_y' ]])
b['L3_OUTLIER'] = np.array(f[['L3_OUTLIER_x','L3_OUTLIER_y' ]])
b['LAST_ERR'] = np.array(f[['LAST_ERR_x', 'LAST_ERR_y']])
b['OUTLIER'] = np.array(f[['OUTLIER_x', 'OUTLIER_y']])
b['REF_BADDAT'] = np.array(f[['REF_BADDAT_x','REF_BADDAT_y' ]])
b['SSIM_AFTER'] = np.array(f[['SSIM_AFTER_x','SSIM_AFTER_y' ]])
b['SSIM_BEFOR'] = np.array(f[['SSIM_BEFOR_x','SSIM_BEFOR_y' ]])
b['SSIM_IMPRO'] = np.array(f[['SSIM_IMPRO_x','SSIM_IMPRO_y' ]])
b['TGT_BADDAT'] = np.array(f[['TGT_BADDAT_x','TGT_BADDAT_y' ]])
b['X_SHIFT_M'] = np.array(f[['X_SHIFT_M_x', 'X_SHIFT_M_y']])
b['X_SHIFT_PX'] = np.array(f[['X_SHIFT_PX_x','X_SHIFT_PX_y' ]])
b['X_WIN_SIZE'] = np.array(f[['X_WIN_SIZE_x','X_WIN_SIZE_y' ]])
b['Y_SHIFT_M'] = np.array(f[['Y_SHIFT_M_x', 'Y_SHIFT_M_y']])
b['Y_SHIFT_PX'] = np.array(f[['Y_SHIFT_PX_x','Y_SHIFT_PX_y' ]])
b['Y_WIN_SIZE'] = np.array(f[['Y_WIN_SIZE_x','Y_WIN_SIZE_y' ]])

b['geometry'] = np.array(f[['geometry_x','geometry_y']])
...
final_composite.to_file(path+'AROSICS_TiePoints_Local_Composite_20200801102721.shp', crs ='EPSG:32632' ) 
...
#create GDAL GCPs
gcps_gdal = [gdal.GCP(row['X'], row['Y'],0, row['Col'], row['Row']) for index, row in gcps_PAN.iterrows()] ### "gcps" or "gcps_PAN"
...
output_image = path+'output_translate_poly3.tif'
...
options = gdal.WarpOptions(dstSRS='EPSG:32632', polynomialOrder=2, targetAlignedPixels=True, xRes=5, yRes =5)
ds = gdal.Warp(path+'PRS_L2D_STD_20200801102717_20200801102721_0001_PAN_Warped_poly1aligned.tif', ds_gcp, dstNodata = np.nan, options=options)
ds_gcp = None
ds = None
